...
function convertStaticModeledMarker(marker) {
    var _a, _b, _c, _d, _e, _f;
    return {
        address: (_a = marker.address) === null || _a === void 0 ? void 0 : _a.value,
        latitude: Number((_b = marker.latitude) === null || _b === void 0 ? void 0 : _b.value),
        longitude: Number((_c = marker.longitude) === null || _c === void 0 ? void 0 : _c.value),
        title: (_d = marker.title) === null || _d === void 0 ? void 0 : _d.value,
        description: (_e = marker.description) === null || _e === void 0 ? void 0 : _e.value,
        onClick: marker.onClick,
        icon: (_f = marker.icon) === null || _f === void 0 ? void 0 : _f.value,
        iconSize: Number(marker.iconSize),
        iconColor: marker.iconColor
    };
}
function convertDynamicModeledMarker(marker) {
    var _a, _b;
    if (marker.markersDS && marker.markersDS.status === "available" /* ValueStatus.Available */) {
        return (_b = (_a = marker.markersDS.items) === null || _a === void 0 ? void 0 : _a.map(item => fromDatasource(marker, item))) !== null && _b !== void 0 ? _b : [];
    }
    return [];
}
function fromDatasource(marker, item) {
    const { locationDynamicType, address, latitude, longitude, onClick, title, description, icon, iconSize, iconColor } = marker;
    return {
...
class Maps extends Component {
    constructor() {
        super(...arguments);
        this.state = {
            status: "loadingMarkers" /* Status.LoadingMarkers */,
            minZoomLevel: 3
        };
    ...
    async componentDidUpdate(prevProps, prevState) {
        if (this.props !== prevProps) {
            ...
            if ((this.state.status === "loadingMarkers" /* Status.LoadingMarkers */ ||
                (this.state.status === "cameraReady" /* Status.CameraReady */ && markersChanged)) &&
                (!this.props.dynamicMarkers.length ||
                    this.props.dynamicMarkers.every(m => { var _a; return ((_a = m.markersDS) === null || _a === void 0 ? void 0 : _a.status) === "available" /* ValueStatus.Available */; }))) {
    ...
    render() {
        return (createElement(View, { style: this.styles.container, testID: this.props.name },
            this.state.status !== "loadingMarkers" /* Status.LoadingMarkers */ ? (createElement(MapView, { ref: this.mapViewRef, provider: this.props.provider === "default" ? null : this.props.provider, mapType: this.props.mapType, showsUserLocation: this.props.showsUserLocation, showsMyLocationButton: this.props.showsUserLocation, showsTraffic: false, minZoomLevel: this.state.minZoomLevel, maxZoomLevel: toZoomValue(this.props.maxZoomLevel), rotateEnabled: this.props.interactive, scrollEnabled: this.props.interactive, pitchEnabled: false, zoomEnabled: this.props.interactive, style: { flex: 1, alignSelf: "stretch" }, liteMode: !this.props.interactive, cacheEnabled: !this.props.interactive, showsPointsOfInterest: false, mapPadding: { top: 48, right: 48, bottom: 48, left: 48 }, onMapReady: this.onMapReadyHandler, onRegionChangeComplete: this.onRegionChangeCompleteHandler }, this.state.markers ? this.state.markers.map(marker => this.renderMarker(marker)) : null)) : null,
            this.state.status === "loadingMarkers" /* Status.LoadingMarkers */ || this.state.status === "loadingMap" /* Status.LoadingMap */ ? (createElement(View, { style: this.styles.loadingOverlay },
                createElement(ActivityIndicator, { color: this.styles.loadingIndicator.color, size: "large" }))) : null));
    }
    renderMarker({ key, props, coordinate }) {
        return (createElement(Marker, { key: key, coordinate: coordinate, title: this.props.interactive ? props.title : "", description: this.props.interactive ? props.description : "", onPress: this.props.interactive ? () => onMarkerPress(props.onClick) : undefined, pinColor: props.iconColor || this.styles.marker.color, opacity: this.styles.marker.opacity },
            props.icon && (createElement(Icon, { icon: props.icon, color: props.iconColor || this.styles.marker.color, size: props.iconSize })),
            createElement(Callout, null,
                createElement(View, null, this.props.popup))));
    }
    async onMapReady() {
    ...
    async onRegionChangeComplete() {
        {
            switch (this.state.status) {
                case "loadingMap" /* Status.LoadingMap */:
                    this.setState({ status: "mapReady" /* Status.MapReady */ });
                    await this.updateCamera(false);
                    break;
                case "mapReady" /* Status.MapReady */:
                    this.setState({
                        status: "cameraReady" /* Status.CameraReady */
                    });
                    break;
            }
        }
    ...
    async parseMarkers() {
        ...
        const parsedMarkers = await Promise.all(markers.map(async (marker, index) => ({
        ...
        this.setState({
            ...
            status: this.state.status === "loadingMarkers" /* Status.LoadingMarkers */ ? "loadingMap" /* Status.LoadingMap */ : this.state.status,
            markers: parsedMarkers
        }, async () => {
            if (this.state.status === "cameraReady" /* Status.CameraReady */) {
                await this.updateCamera(true);
            }
        });
    }
    async updateCamera(animated) {
        ...
        if (this.props.fitToMarkers && this.state.markers && this.state.markers.length > 1) {
            const coords = this.state.markers.map(marker => marker.coordinate);
    ...
    async getCenter() {
        ...
        const center = (isValidCoordinate(centerLatitude === null || centerLatitude === void 0 ? void 0 : centerLatitude.value) && isValidCoordinate(centerLongitude === null || centerLongitude === void 0 ? void 0 : centerLongitude.value)) || centerAddress
            ? await this.parseCoordinate(Number(centerLatitude === null || centerLatitude === void 0 ? void 0 : centerLatitude.value), Number(centerLongitude === null || centerLongitude === void 0 ? void 0 : centerLongitude.value), centerAddress === null || centerAddress === void 0 ? void 0 : centerAddress.value)
            : ((_a = this.state.markers) === null || _a === void 0 ? void 0 : _a.length) === 1 && fitToMarkers
                ? this.state.markers[0].coordinate
                : { latitude: 51.9066346, longitude: 4.4861703 };
        return center;
    }
    parseCoordinate(latitude, longitude, address) {
        if (isValidCoordinate(latitude) && isValidCoordinate(longitude)) {
            ...
            throw new Error(`Address: "${address}", Latitude: "${latitude}", Longitude: "${longitude}". None of these values could be parsed to coordinates.`);
        }
    }
}
function didMarkersChange(prevMarkers, newMarkers, newDynamicMarkers) {
    const markers = newMarkers.flatMap(convertStaticModeledMarker);
    const dynamicMarkers = newDynamicMarkers.flatMap(convertDynamicModeledMarker);
    const combinedMarkers = [...dynamicMarkers, ...markers];
    return ((prevMarkers === null || prevMarkers === void 0 ? void 0 : prevMarkers.length) !== (combinedMarkers === null || combinedMarkers === void 0 ? void 0 : combinedMarkers.length) ||
        !!(combinedMarkers === null || combinedMarkers === void 0 ? void 0 : combinedMarkers.filter((marker) => (prevMarkers === null || prevMarkers === void 0 ? void 0 : prevMarkers.filter((prevMarker) => {
            ...
            return true;
        }).length) !== prevMarkers.length).length));
}
